...
int main(int argc, char *argv[])
{
    ...
    int on = 1, retuval, cli_addr_len, recv_len, count = 0, old;
    char buf[BUFF_SIZE];
    ...
    //设为非阻塞状态
    old = fcntl(listen_fd, F_GETFL);
    fcntl(listen_fd, F_SETFL, O_NONBLOCK | old);
            ...
            for (int i = 0; i < retuval; ++i) {
                if (events[i].events & EPOLLIN) {
                    if (events[i].data.fd != listen_fd) {
                        ...
                        //设为非阻塞状态
                        old = fcntl(new_fd, F_GETFL);
                        fcntl(new_fd, F_SETFL, O_NONBLOCK | old);
